...
| `mgcv::gam` + `s(id, bs="re")` (additive panel model) | 1.117 | 1.283 |
...
estimation one, and `pannet` closes it by a replicated 17-24% margin,
consistently across both DGPs and all 5 seeds each.

The fair *panel* comparator isn't a pooled GAM (that has no individual
effects at all); it's a GAMM: `gam(y ~ s(x1) + s(x2) + x3 + s(id,
bs="re"))`, which adds smooth terms *and* panel structure. Measured
against that, `pannet` ties it on the harder DGP and trails it slightly
on the easier one. That's an honest result, not a knock-down win, and
worth stating plainly: when you already know which variables enter
nonlinearly and can name a smooth-additive form, a GAMM matches `pannet`
at that accuracy with an interpretable, well-understood fit. `pannet`'s
case is that it doesn't require specifying that form in advance: the MLP
learns the shape (including interactions a purely additive GAMM doesn't
represent without you naming them), not that it's more accurate than a
correctly-specified additive alternative. See `vignette("pannet-validation")`
for the full methodology and both comparators.
...
more than the effect itself, so refit across a few seeds).
